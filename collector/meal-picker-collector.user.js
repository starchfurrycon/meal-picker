...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     必须在页面主世界执行才能包到真正的 fetch / XHR，
     所以把函数序列化后插进 <script>，结果用 postMessage 送回。
     ══════════════════════════════════════════════════════════ */
  ...
  window.addEventListener('message', (ev) => {
    ...
    if (found.length) {
      ...
      const offers = found.slice(0, 30).map((r) => normalizeRecord({ ...r, api: d.url.slice(0, 160) }, kw));
      collected.push(...offers);
      collectedSources.add(d.url.slice(0, 160));
      lastHarvestAt = Date.now();
      LOG(`从接口拿到 ${offers.length} 条候选`, d.url.slice(0, 100));
    } else {
      LOG('接口响应里没识别出候选', d.url.slice(0, 100), '（可把这条响应存下来调适配）');
    }
  });
  ...
  function gmRequest(opts) {
    return new Promise((resolve, reject) => {
      ...
      // 没有 GM 权限时退化为普通 fetch（仅当页面本身是 http 或允许时才通）
      fetch(opts.url, {
        method: opts.method || 'GET',
        headers: opts.headers || { 'Content-Type': 'application/json' },
        body: opts.body,
  ...
  async function relayAlive() {
    try {
      const r = await gmRequest({ url: `${RELAY}/api/health`, timeout: 3000 });
      return r.ok;
    } catch { return false; }
  }
  ...
  async function fetchTask() {
    try {
      const r = await gmRequest({ url: `${RELAY}/api/task?platform=${PLATFORM}`, timeout: 4000 });
      if (!r.ok) return null;
